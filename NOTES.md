...
23. **6 個研究引入的 detector 預設全部不啟用**（docs/strategy-research.md）：
    符合規格「reversal 預設關閉：要開請自己回測過再說」的同一精神——
    先 `run.py evaluate` 用自己的資料驗證，通過門檻再手動加進 enabled_detectors。
    系統永遠不會自動改 config。
24. **Connors 系列原版不設停損**，本系統統一套 ATR 停損 + RR 目標引擎回測，
    屬刻意保守偏離；文獻勝率數字不可直接對照。
25. **evaluate 只驗證典型參數、不做參數搜尋**：多輪（walk-forward 分段 × 跨標的）
    驗證用來淘汰不穩健策略，不是用來調參數——對應規格 §11 對過擬合的警告。
26. **月線級策略（TSMOM、Faber、黃金交叉）不實作**：單檔 500 根 K 訊號 1-3 次，
    永遠過不了 min_samples，實作了也只會被壓在 45 分，徒增雜訊。
